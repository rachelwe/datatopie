 ...
 * @version 0.1
 */class Point{constructor(t,e){this.x=t;this.y=e}toSvgPath(){return`${this.x} ${this.y}`}static fromAngle(t){return new Point(Math.cos(t),Math.sin(t))}}class GraphPie{constructor(t,e){this.datas=t;this.config=e;this.nodes={};this.paths={};this.tooltips={}}debug(){console.log(this)}_setTemplate(){this.nodes.donut=stringToDom(`<div class="graph_wrapper_svg">\n        <svg class="graph" aria-labelledby="${this.config.id}-title" id="${this.config.id}" viewBox="-1 -1 2 2">\n          <title id="${this.config.id}-title">${this.config.mainTitle}</title>\n          <g class="graph_data" data-datas data-hide="" mask="url(#graphMask)">\n          </g>\n          <mask id="graphMask">\n              <rect fill="white" x="-1" y="-1" width="2" height="2"/>\n              <circle r="${this.config.donut||.5}" fill="black"/>\n          </mask>\n        </svg>\n        <div class="graph_tooltips" data-tooltips></div>\n      </div>`);this.nodes.legend=stringToDom(`<div class="graph_wrapper_legend"></div>`);this.nodes.svg=_select(".graph",this.nodes.donut);this.nodes.data=_select("[data-datas]",this.nodes.donut);this.nodes.tooltips=_select("[data-tooltips]",this.nodes.donut)}_setPaths(){this.paths=this.datas.map(((t,e)=>_createSVGElement(this.nodes.data,"path",[{fill:t.color},{"data-value":t.value},{"data-label":t.label},{id:this.config.id+"-path-"+e},{"aria-labelledby":this.config.id+"-tooltip-"+e},{tabindex:0}])))}_setTooltips(){this.tooltips=this.datas.map(((t,e)=>_createHTMLElement(this.nodes.tooltips,"div",[{class:"graph_tooltip"},{"data-value":t.value},{"data-color":t.color},{id:this.config.id+"-tooltip-"+e}],`<p>${t.label}</p>\n      <p>${t.value}</p>\n    `)))}_setLegend(){const t=this.datas.reduce(((t,e)=>t+e.value),0);this.legend=this.datas.map(((e,s)=>_createHTMLElement(this.nodes.legend,"div",[{class:"graph_legend"},{"data-value":e.value},{style:"--data-color:"+e.color+";--data-percentage:"+e.value/t*100},{"data-index":s},{"data-display":true},{id:this.config.id+"-legend-"+s}],`<div class="graph_legend_label"><p>${e.label}</p>\n        <p id="${this.config.id+"-legend-meter-"+s}">${e.value}&nbsp;${this.config.unite}</p></div>\n      \x3c!-- <meter min="0" max="${t}" value="${e.value}">${e.value} ${e.label}</meter> --\x3e\n      <div class="meter" role="meter" aria-valuenow="${e.value}" aria-valuemin="0" aria-valuemax="${t}" aria-labelledby="${this.config.id+"-legend-meter-"+s}"></div>\n      <button data-toggle>${this.config.hideButton}</button>\n    `)))}_positionLabel(t,e){if(!t||!e){return}const s=Point.fromAngle(e);t.style.setProperty("top",`${(s.y*.5+.5)*100}%`);t.style.setProperty("left",`${(s.x*.5+.5)*100}%`)}draw(t=this.datas,e=1){const s=t.reduce(((t,e)=>t+e.value),0);let a=Math.PI/-2;let i=new Point(0,-1);for(let n=0;n<t.length;n++){const l=t[n].value/s*e;if(e===1){this._positionLabel(this.tooltips[n],a+l*Math.PI)}a+=l*2*Math.PI;const o=Point.fromAngle(a);const r=l>.5?"1":"0";this.paths[n].setAttribute("d",`M 0 0 L ${i.toSvgPath()} A 1 1 0 ${r} 1 ${o.toSvgPath()} L 0 0`);i=o}}animate(t=this.datas){const e=Date.now();const s=1e3;const a=()=>{const i=(Date.now()-e)/s;if(i<1){this.draw(t,easeOutExpo$1(i));window.requestAnimationFrame(a)}else{this.draw(t,1)}};window.requestAnimationFrame(a)}_toggleElement(t){if(!t.target.closest(".graph_legend button"))return;const e=t.target.closest(".graph_legend");const s=e.getAttribute("data-display");const a=parseInt(e.getAttribute("data-index"));const i=this.datas.find(((t,e)=>e===a));if(s==="true"){e.setAttribute("data-display","false");i.display=false}else if(s==="false"){e.setAttribute("data-display","true");delete i.display}const n=this.datas.map(((t,e)=>{const s={...t};if(s.display===false){s.value=0}return s}));this.draw(n)}_events(){["mouseenter","focus"].forEach((t=>{this.nodes.svg.addEventListener(t,(t=>{if(t.target.matches("path")){const e=t.target.getAttribute("aria-labelledby");const s=document.getElementById(e);s.classList.add("is-active")}}),true)}));["mouseleave","blur"].forEach((t=>{this.nodes.donut.addEventListener(t,(t=>{const e=t.target.matches("path");const s=t.target.closest(".graph_tooltip");const a=t.relatedTarget?t.relatedTarget.matches("path"):false;const i=t.relatedTarget?t.relatedTarget.closest(".graph_tooltip"):false;if(e&&!i){const e=t.target.getAttribute("aria-labelledby");const s=document.getElementById(e);s.classList.remove("is-active")}else if(s&&!a){s.classList.remove("is-active")}}),true)}));this.config.wrapper.addEventListener("click",(t=>{this._toggleElement(t)}),true)}init(){this._setTemplate();this._setPaths();this._setTooltips();this._setLegend();this.config.wrapper.appendChild(this.nodes.donut);this.config.wrapper.appendChild(this.nodes.legend);this.animate();this._events()}}customElements.define("pie-chart",PieChart);new GraphLine({},{});const pieChart={config:{mainTitle:"A very simple pie chart about vegan diet",legendKey:"label",wrapper:document.querySelector('[data-graph="pie"]'),id:Date.now(),donut:.8,unite:"parts",hideButton:'<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24"><path fill-rule="evenodd" d="M2.1 12a18.7 18.7 0 0 0 2.5 3.3A10 10 0 0 0 12 19c3.1 0 5.6-1.8 7.4-3.7a18.7 18.7 0 0 0 2.5-3.3 18.7 18.7 0 0 0-2.5-3.3A10 10 0 0 0 12 5C8.9 5 6.4 6.8 4.6 8.7A18.7 18.7 0 0 0 2.1 12zM23 12l.9-.4a10.6 10.6 0 0 0-.2-.4 20.7 20.7 0 0 0-2.8-3.9c-2-2-5-4.3-8.9-4.3-3.9 0-6.9 2.2-8.9 4.3a20.7 20.7 0 0 0-2.8 3.9 12.4 12.4 0 0 0-.2.3s0 0 .9.5l-.9-.4a1 1 0 0 0 0 .8L1 12l-.9.4a8.3 8.3 0 0 0 .2.4 20.7 20.7 0 0 0 2.8 3.9c2 2 5 4.3 8.9 4.3 3.9 0 6.9-2.2 8.9-4.3a20.7 20.7 0 0 0 2.8-3.9 11.8 11.8 0 0 0 .2-.3s0 0-.9-.5zm0 0 .9.4a1 1 0 0 0 0-.8l-.9.4z" clip-rule="evenodd"/><path fill-rule="evenodd" d="M12 10a2 2 0 1 0 0 4 2 2 0 0 0 0-4zm-4 2a4 4 0 1 1 8 0 4 4 0 0 1-8 0z" clip-rule="evenodd"/></svg>'},data:[{label:"Legumes",value:3,color:"blue"},{label:"Grains",value:5,color:"peru"},{label:"Fruits",value:2,color:"red"},{label:"Nuts, oil",value:1,color:"yellow"},{label:"Vegetables",value:4,color:"green"}]};const graphPie=new GraphPie(pieChart.data,pieChart.config);graphPie.init();graphPie.debug();